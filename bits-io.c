/********************************************************************
 ...
 WRITING BITS
   ...
   routine deal with it.  This this does not impact writing bytes, but it will
 ...
 READING BITS
   ...
   and to shift marker bits into the low end.  We will do this is such a way
   ...
   the high order bit is a 0 (as for the full-byte case), and then one more
   (also as for the full byte case).  Here is an example of a patial byte
...
int bits_io_read_bit (BitsIOFile *bfile) {
  ...
  if(bfile->byte == ALL_BITS_READ){          //get new byte if needed
    ...
    bfile->count++;
    int shift_in = 1;
    int bit = (((bfile->byte) >> 7) & 1);     
    while(bit == 1){                          //if byte isn't full, need to get ride of leading 1's
      bfile->byte <<= 1;
      bfile->byte = bfile->byte | shift_in;   //puts 1 in for delimiter when first run
      bit = (((bfile->byte) >> 7) & 1);      
      shift_in = 0;
    }
    bfile->byte <<= 1;                        //get rid of leading zero
    bfile->byte = bfile->byte | shift_in;
    shift_in = 0;

  }

  int return_bit = (((bfile->byte) >> 7) & 1);
  bfile->byte <<= 1;
