...
static void compute_freq (FILE *fp, Context *ctx) {
  ...
  // through the all the legal entries.  Next, we iterate over the input to
  ...
  // more details.  You should make sure that the character you read in is in
  // the range of the characters we are considering.  If it is in the range
  // you must update its entry in the frequency table by incrementing its
  // frequency.
  for(int j=0; j<NUMBER_OF_CHARS; j++){           //sets table indicies to default
    ctx -> table[j].v = 0;
    ctx -> table[j].c = j;
  }
  char cIn;
  while(cIn != EOF){
    cIn = fgetc(fp);                              //get next char
    if(cIn >= 0 && cIn <= 127){                   //check to see if char is ascii
      for(int i=0; i<NUMBER_OF_CHARS; i++){       //loop on char array
        if(cIn == (ctx -> table[i].c)){           //char exists
          ctx -> table[i].v++;
          break;                                  //done with char
        }
        else if(i == (ctx -> table[i].c)){        //char doesnt exsist
          ctx -> table[i].c = cIn;
          ctx -> table[i].v++;
          break;                                  //done with char
        }
...
static void create_tree_nodes (Context *ctx) {
  ...
  for(int i=0; i<=NUMBER_OF_CHARS; i++){        //loop through table
    if(ctx -> table[i].v > 0){                  
      TreeNode *tn = tree_new();                //new tree node for pqueue
      tn -> freq.v = ctx -> table[i].v;
      tn -> freq.c = ctx -> table[i].c;
      tn -> type = LEAF;
      pqueue_enqueue(ctx -> pq, tn);
    }
    else { return; }                            // v =< 0 at end of table
  }
...
static TreeNode *build_tree (Context *ctx) {
  ...
  while(pqueue_size(ctx -> pq) > 1){
    TreeNode *tn_L = pqueue_dequeue(ctx -> pq);
    TreeNode *tn_R = pqueue_dequeue(ctx -> pq);
    TreeNode *tn_N = tree_new();
    tn_N -> freq.v = tn_L -> freq.v + tn_R -> freq.v;
    tn_N -> left = tn_L;
    tn_N -> right = tn_R;
    pqueue_enqueue(ctx -> pq , tn_N);
  }
